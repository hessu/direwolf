...
static void entry_group_callback(AvahiEntryGroup *g, AvahiEntryGroupState state, AVAHI_GCC_UNUSED void *userdata)
{
    ...
    switch (state) {
        case AVAHI_ENTRY_GROUP_ESTABLISHED :
            ...
            dw_printf("Avahi: Service '%s' successfully established.\n", name);
            break;
        case AVAHI_ENTRY_GROUP_COLLISION : {
            ...
            avahi_free(name);
            name = n;
            dw_printf("Avahi: Service name collision, renaming service to '%s'\n", name);
            /* And recreate the services */
            create_services(avahi_entry_group_get_client(g));
            break;
        }
        case AVAHI_ENTRY_GROUP_FAILURE :
            dw_printf("Avahi: Entry group failure: %s\n", avahi_strerror(avahi_client_errno(avahi_entry_group_get_client(g))));
...
static void create_services(AvahiClient *c) {
    ...
    if (!group) {
        if (!(group = avahi_entry_group_new(c, entry_group_callback, NULL))) {
            dw_printf("Avahi: avahi_entry_group_new() failed: %s\n", avahi_strerror(avahi_client_errno(c)));
            goto fail;
        }
    } else {
    ...
    if (avahi_entry_group_is_empty(group)) {
        dw_printf("Avahi: Adding service '%s'\n", name);
        ...
        if ((ret = avahi_entry_group_commit(group)) < 0) {
            dw_printf("Avahi: Failed to commit entry group: %s\n", avahi_strerror(ret));
            goto fail;
        }
    }
    return;
...
static void client_callback(AvahiClient *c, AvahiClientState state, AVAHI_GCC_UNUSED void * userdata)
{
    ...
    switch (state) {
        ...
        case AVAHI_CLIENT_FAILURE:
            dw_printf("Avahi: Client failure: %s\n", avahi_strerror(avahi_client_errno(c)));
            avahi_simple_poll_quit(simple_poll);
            break;
...
void dns_sd_announce (struct misc_config_s *mc)
{
	...
	dw_printf ("dns_sd_announce ( %d )\n", kiss_port);
	
	int error;
	
	/* Allocate main loop object */
	if (!(simple_poll = avahi_simple_poll_new())) {
	    dw_printf ("dns_sd_announce: Failed to create Avahi simple poll object.\n");
	    goto fail;
        }
        ...
        if (!client) {
            dw_printf ("dns_sd_announce: Failed to create Avahi client: %s\n", avahi_strerror(error));
            goto fail;
        }
